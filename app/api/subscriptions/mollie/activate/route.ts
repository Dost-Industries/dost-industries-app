...
export async function POST(
  ...
  try {
    ...
    /*
     * Always re-fetch and verify the
     * original Premium payment before
     * either creating or recovering a
     * subscription. An existing active
     * Mollie subscription must never be
     * enough to reactivate access after
     * that first payment was fully
    ...
    const financialBillingUpdate:
      Record<string, unknown> = {};

    applyPremiumFinancialState(
      financialBillingUpdate,
      payment.id,
      financialState
    );
    ...
    if (
      ...
      return NextResponse.json(
        {
          error:
            "The DOST Premium payment has been financially revoked.",
     ...
     * another one. The first payment
     * has already passed the financial
     * revocation check above.
     */
